...
import { useUpdateGigShare } from "@/lib/gigs.queries";
...
function publicUrl(slug: string | null): string | null {
  if (!slug) return null;
  return `${window.location.origin}/gigs/g/${slug}`;
}

export function GigShareDialog({
  gigId,
  open,
  onOpenChange,
  initialEnabled,
  ...
  const update = useUpdateGigShare(gigId);
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Share2 className="size-4" /> Share this gig
          </DialogTitle>
          <DialogDescription>
            Anyone with this link can view your gig in read-only mode. Disable anytime to invalidate.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 px-6 py-4">
          <label className="flex items-center justify-between gap-3 rounded-md border border-border bg-card p-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                {enabled
                  ? `${viewCount} view${viewCount === 1 ? "" : "s"} so far.`
                  : "Currently disabled — slug invalidates when re-enabled."}
              </p>
            </div>
            <input
              type="checkbox"
          ...
          {enabled && url && (
            <div className="space-y-2">
              <Input value={url} readOnly aria-label="Public share URL" />
              ...
              <p className="text-[11px] text-muted-foreground">
                Warning: anyone with this link sees your full gig. Disable to revoke access.
              </p>
            </div>
          )}
