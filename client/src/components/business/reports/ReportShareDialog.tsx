...
import { useUpdateReportShare } from "@/lib/reports.queries";
...
function publicUrl(slug: string | null): string | null {
  if (!slug) return null;
  return `${window.location.origin}/reports/r/${slug}`;
}
...
export function ReportShareDialog({
  ...
  const update = useUpdateReportShare(reportId);
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Share2 className="size-4" /> Share this report
          </DialogTitle>
          <DialogDescription>
            Anyone with this link can view this report. Disable anytime.
          </DialogDescription>
        </DialogHeader>
